...
from pocknix_control.power import set_cpu_governor, set_power_profile
...
class Plugin:
    ...
    async def set_cpu_governor(self, governor):
        await asyncio.to_thread(set_cpu_governor, governor)
        return await self.get_config()

    async def set_power_profile(self, profile):
        await asyncio.to_thread(set_power_profile, profile)
        return await self.get_config()
