...
const setCpuGovernor = (gov) => call("set_cpu_governor", gov);
const setPowerProfile = (profile) => call("set_power_profile", profile);
...
const governorOptions = [
    ...
    { data: "schedutil", label: "Schedutil" },
    { data: "performance", label: "Performance" },
    { data: "ondemand", label: "Ondemand" },
];
const powerProfileOptions = [
    { data: "bajo", label: "Low" },
    { data: "medio", label: "Medium" },
    { data: "alto", label: "High (no limit)" },
];
...
function PerfFields({ values, patch }) {
    ...
    const perGameGovernor = [globalChoice, ...governorOptions];
    const perGameProfile = [globalChoice, ...powerProfileOptions];
    ...
    const govValue = perGameGovernor.some((option) => option.data === String(values.cpuGovernor ?? "")) ? String(values.cpuGovernor ?? "") : "";
    const profileValue = perGameProfile.some((option) => option.data === String(values.powerProfile ?? "")) ? String(values.powerProfile ?? "") : "";
    return (SP_JSX.jsxs(SP_JSX.Fragment, { children: [SP_JSX.jsx(SelectEdit, { label: "CPU Scheduler", value: lavdValue, options: perGameLavd, onChange: (id) => patch({ lavdMode: id }) }), SP_JSX.jsx(SelectEdit, { label: "CPU Cores", value: pinValue, options: cpuPinOptions, onChange: (id) => patch({ cpuPin: id }) }), SP_JSX.jsx(SelectEdit, { label: "Fan Curve", value: fanValue, options: perGameFan, onChange: (id) => patch({ fanMode: id }) }), SP_JSX.jsx(SelectEdit, { label: "CPU Governor", value: govValue, options: perGameGovernor, onChange: (id) => patch({ cpuGovernor: id }) }), SP_JSX.jsx(SelectEdit, { label: "Power Profile", value: profileValue, options: perGameProfile, onChange: (id) => patch({ powerProfile: id }) })] }));
}
...
function Games({ config, setConfig, reload }) {
    ...
    return (SP_JSX.jsxs(SP_JSX.Fragment, { children: [SP_JSX.jsxs(DFL.PanelSection, { title: "PERFORMANCE & GAME TWEAKS", children: [SP_JSX.jsx(SelectEdit, { label: "Game", value: game?.appid || "", options: editTargetOptions(config), onChange: setSelectedGame }), !editingDefault ? SP_JSX.jsx(DFL.ToggleField, { label: "Use Per-Game Settings", checked: perGameEnabled, onChange: setPerGameEnabled }) : null] }), showFields ? (SP_JSX.jsx(DFL.PanelSection, { title: "PERFORMANCE", children: editingDefault ? (SP_JSX.jsxs(SP_JSX.Fragment, { children: [SP_JSX.jsx(SelectEdit, { label: "CPU Scheduler", value: config.lavdMode, options: lavdOptions, onChange: (mode) => applyMode(setLavdMode, mode) }), SP_JSX.jsx(SelectEdit, { label: "Fan Curve", value: config.fanMode, options: fanOptions, onChange: (mode) => applyMode(setFanMode, mode) }), SP_JSX.jsx(SelectEdit, { label: "CPU Governor", value: config.cpuGovernor, options: governorOptions, onChange: (mode) => applyMode(setCpuGovernor, mode) }), SP_JSX.jsx(SelectEdit, { label: "Power Profile", value: config.powerProfile, options: powerProfileOptions, onChange: (mode) => applyMode(setPowerProfile, mode) })] })) : (SP_JSX.jsx(PerfFields, { values: values, patch: patchSettings })) })) : null, showFields ? (SP_JSX.jsxs(DFL.PanelSection, { title: "GAME TWEAKS", children: [SP_JSX.jsx("div", { className: "pocknix-note", children: "Changes apply on next game launch" }), editingDefault ? (SP_JSX.jsxs(SP_JSX.Fragment, { children: [SP_JSX.jsx(SelectEdit, { label: "FEX Preset", value: fexValue, options: fexOptions, onChange: (id) => {
